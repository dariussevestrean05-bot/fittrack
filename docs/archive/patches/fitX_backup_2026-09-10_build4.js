...
function WaterCard({ water, setWater, nutrition, setNutrition }) {
  ...
  // Rechner wie bei kcal: 35 ml/kg + Aktivitäts-Bonus, speichert waterGoal in nutrition.
  const calcWater = () => {
    const v = recommendWaterGoal(nutrition || {});
    if (!v) {
      setWaterInfo("Trage zuerst dein Gewicht ein (oben in Ernährung) – daraus wird dein Bedarf berechnet.");
      return;
    }
    setNutrition({ ...(nutrition || {}), waterGoal: v });
    setWaterInfo(`Empfohlen: ${v} ml/Tag (35 ml/kg + Aktivität).`);
  };
  ...
  return (
    <div className="stat-card wide card-in" style={{ marginTop: 8 }}>
      ...
      <button className="link-btn" style={{ marginTop: 6 }} onClick={calcWater}>🧮 Wasserziel ausrechnen lassen</button>
      {waterInfo && <div className="row-sub">{waterInfo}</div>}
...
function FoodTab({ meals, setMeals, nutrition, setNutrition, profile, water, setWater }) {
  ...
  return (
    <div className="pad">
      ...
      <WaterCard water={water} setWater={setWater} nutrition={nutrition} setNutrition={setNutrition} />
