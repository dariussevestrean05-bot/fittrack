...
export default function App() {
  ...
  // Android Back-Button (SPA): Banner schließen, sonst zurück zu Start statt App-Exit.
  useEffect(() => {
    const onBack = () => {
  ...
  return (
    <div className="pad home-cover">
      <div className="hero-card clean-cover card-in">
        ...
        {!nextEvent && <button className="hero-cta" onClick={() => goTo("ziele")}>Erinnerung anlegen</button>}
      </div>
...
// Ziele vereint Check-in, Erinnerungen, Skill-Baum und gemeinsame Vorschläge
// (ehem. eigener Erinnerungs-Tab – 5 statt 6 Icons in der Tabbar).
function ZieleTab({ checkins, setCheckins, skills, setSkills, reminders, setReminders, proposals, persistProposals, profile }) {
  return (
    <div className="pad">
      <div className="header-row">
      ...
      <ProposalSection proposals={proposals} persistProposals={persistProposals} profile={profile} />
    </div>
  );
}

// ---------- Erinnerungen (Section in Ziele) ----------
function ReminderSection({ reminders, setReminders }) {
  ...
  return (
    <div>
      <div className="section-head">
        ...
        <button className="mini-btn" onClick={() => setAdding(!adding)} aria-label={adding ? "Abbrechen" : "Erinnerung hinzufügen"}>{adding ? <X size={15} /> : <Plus size={15} />}</button>
      </div>
      ...
      {safeReminders.length === 0 && !adding && <EmptyHint text="Keine Erinnerungen gestellt." />}
      {safeReminders.map((r) => (
...
// ---------- Gemeinsame Trainings (Section in Ziele) ----------
function ProposalSection({ proposals, persistProposals, profile }) {
  ...
  return (
    <div>
      <div className="section-title">Gemeinsame Trainings</div>
...
// ---------- Profil ----------
// Einzige Stelle für persönliche Daten (Name/Alter/Geschlecht) – bewusst getrennt von Zielen/Training/Ernährung.
...
function ProfilTab({ profile, setProfile, checkins, workouts, skills, nutrition }) {
  ...
  return (
    <div className="pad">
      ...
      <div className="row-sub" style={{ marginBottom: 8 }}>Hier – und nur hier – liegen deine persönlichen Daten. Ziele/Training/Ernährung nutzen sie nur.</div>
      <div className="form-card card-in">
