...
// Körperbau statt KFA-Raten: kaum einer kennt seinen Fettanteil – nur ob dünn oder kräftig.
const BUILD_LABELS = { schlank: "Schlank", normal: "Normal", kraeftig: "Kräftig" };
// Grobe KFA-Schätzung (%) aus Körperbau + Geschlecht, nur für Katch-McArdle-Fallback.
function estimateBodyFat(build, gender) {
  const table = {
    weiblich: { schlank: 20, normal: 27, kraeftig: 35 },
    maennlich: { schlank: 12, normal: 18, kraeftig: 26 },
  };
  const row = table[gender] || { schlank: 16, normal: 22, kraeftig: 30 };
