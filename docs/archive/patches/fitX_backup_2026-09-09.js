...
const THEMES = {
  ...
  light: {
    bg: "#F5F5FA", bgElevated: "rgba(255,255,255,0.75)", bgInput: "rgba(20,23,30,0.04)",
    ...
    danger: "#D8402F", tabbarBg: "rgba(255,255,255,0.7)",
    shadow: "0 25px 60px rgba(30,30,50,0.16)",
    blobA: "rgba(225,72,29,0.16)", blobB: "rgba(106,69,232,0.16)", blobC: "rgba(20,150,220,0.12)",
    bgGlow: "radial-gradient(120% 60% at 50% -10%, rgba(225,72,29,0.10), transparent 60%), radial-gradient(100% 50% at 50% 110%, rgba(106,69,232,0.12), transparent 60%)",
  },
};
function cssVars(t) {
  return {
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      const [r, w, m, th, pr, prop, pl, spl, nu, ci, sk, rd] = await Promise.all([
  ...
  return (
    <div className="app" style={cssVars(t)}>
      ...
      <div className="screen">
        ...
        {tab === "ernaehrung" && (
          <FoodTab meals={meals} setMeals={setMeals} nutrition={nutrition} setNutrition={setNutrition} profile={profile} />
        )}
        {tab === "erinnerungen" && (
          <ReminderTab
      ...
      <nav className="tabbar">
        ...
        <TabBtn icon={Bell} label="Erinnerung" active={tab === "erinnerungen"} onClick={() => setTab("erinnerungen")} />
...
// Sauberes Cover: kein Workouts-gesamt (siehe Training-Tab), keine Makro-Details, kein Verlauf.
// Nur: Hero (nächstes Training) + Streak-Pill + Heute-Kcal kompakt + Quick-Actions.
function HomeTab({ nextEvent, goTo, meals, nutrition, checkins }) {
  ...
  return (
    <div className="pad home-cover">
      ...
      <div className="cover-card card-in" onClick={() => goTo("ernaehrung")} role="button" tabIndex={0}
        onKeyDown={(e) => { if (e.key === "Enter") goTo("ernaehrung"); }}>
        <div className="cover-head"><span>Heute</span><span>{todayKcal} / {targetKcal} kcal</span></div>
        <div className="progress-bar slim"><div className="progress-fill" style={{ width: `${pct}%` }} /></div>
      </div>
...
function TrainingTab({ workouts, setWorkouts, plans, setPlans, sharedPlans, persistSharedPlans, profile, restDays, setRestDays }) {
  ...
  const remove = (id) => setWorkouts((w) => w.filter((x) => x.id !== id));
...
function FoodTab({ meals, setMeals, nutrition, setNutrition, profile }) {
        ...
        <>
          ...
          {preview && (
            <div className="form-card card-in" style={{ marginTop: 12 }}>
              <img src={preview} alt="Mahlzeitenfoto" className="meal-preview" />
...
function CheckinCard({ checkins, setCheckins }) {
  ...
  const remove = (id) => setCheckins(safe.filter((c) => c && c.id !== id));
  ...
  return (
    <div className="form-card card-in">
      ...
      {!pendingPhoto ? (
        ...
        <>
          <img src={pendingPhoto} alt="Gym-Foto zur Prüfung" className="meal-preview" />
          ...
          return (
            <div key={s.id}>
              ...
              >
                ...
                {done && entry && entry.photo && <img src={entry.photo} alt="" className="tree-thumb" />}
              </button>
...
function ReminderTab({ reminders, setReminders, proposals, persistProposals, profile }) {
  ...
  const removeReminder = (id) => setReminders(safeReminders.filter((r) => r && r.id !== id));
  ...
  return (
    <div className="pad">
      <div className="header-row">
        ...
        <button className="icon-btn" onClick={() => setAdding(!adding)}>{adding ? <X size={18} /> : <Plus size={18} />}</button>
      </div>
      ...
      {safeReminders.map((r) => (
        <div key={r.id} className="row-card card-in">
          ...
          <button className="icon-btn-ghost" onClick={() => removeReminder(r.id)}><Trash2 size={14} /></button>
        </div>
      ))}
      ...
      {safeProposals.map((p) => (
        <div key={p.id} className="row-card card-in">
          ...
          <button className="icon-btn-ghost" onClick={() => removeProposal(p.id)}><Trash2 size={14} /></button>
        </div>
      ))}
    </div>
  );
...
// ---------- Profil ----------
// Einzige Stelle für persönliche Daten (Name/Alter/Geschlecht) – bewusst getrennt von Erinnerung/Training/Ernährung.
...
function ProfilTab({ profile, setProfile, checkins, workouts, skills, nutrition }) {
  ...
  return (
    <div className="pad">
      ...
      <div className="row-sub" style={{ marginBottom: 8 }}>Hier – und nur hier – liegen deine persönlichen Daten. Erinnerung/Training/Ernährung nutzen sie nur.</div>
      <div className="form-card card-in">
...
.check-line { display: flex; align-items: center; gap: 10px; width: 100%; text-align: left; background: var(--bg-input); border: 1px solid var(--border); color: var(--text); padding: 10px 12px; border-radius: 12px; font-size: 0.83rem; cursor: pointer; }
